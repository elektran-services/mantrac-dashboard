...
import fs from 'fs';
import { purgeExpiredOfflineReports, resolveSafeOfflineReportPath } from '@/lib/offlineReportStorage';
...
/**
 * GET /api/offline-reports/download?file=offline_daily_report_2026-05-06.xlsx
 */
export async function GET(request: NextRequest) {
  ...
  const resolved = resolveSafeOfflineReportPath(file.trim());
  if (!resolved) {
    return NextResponse.json({ status: -1, cause: 'File not found' }, { status: 404 });
  }

  const buf = fs.readFileSync(resolved);
  const name = file.trim().split(/[/\\]/).pop() || 'offline-report.xlsx';
