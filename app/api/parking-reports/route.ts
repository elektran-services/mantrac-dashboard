...
import {
  filterParkingReportsByDateParams,
  listParkingReportFiles,
  PARKING_REPORTS_RETENTION_DAYS,
 ...
 * Body: { token?, date?, from?, to? } — list saved daily parking Excel files from parking_reports/.
 */
export async function POST(request: NextRequest) {
  ...
  const all = listParkingReportFiles();
  const filtered = filterParkingReportsByDateParams(all, { date, from, to });

  return NextResponse.json({
    status: 0,
    cause: 'OK',
    retentionDays: PARKING_REPORTS_RETENTION_DAYS,
    files: filtered,
