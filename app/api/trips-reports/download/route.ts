...
import fs from 'fs';
import { resolveSafeTripReportPath, purgeExpiredTripReports } from '@/lib/tripsReportStorage';
...
/**
 * GET /api/trips-reports/download?file=trips_daily_report_2026-05-01.xlsx
 */
export async function GET(request: NextRequest) {
  ...
  const resolved = resolveSafeTripReportPath(file.trim());
  if (!resolved) {
